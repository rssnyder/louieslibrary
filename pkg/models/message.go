...
func (db *DB) GetConversation(sender, reciver string) (Messages, error) {
	...
	// Query statement
	stmt := `SELECT sender, reciver, read, content, created FROM messages WHERE sender = $1 AND reciver = $2 OR  sender = $2 AND reciver = $1`
...
func (db *DB) GetThreads(reciver string) (Messages, error) {
	...
	stmt := `SELECT DISTINCT sender, reciver FROM messages WHERE reciver = $1 OR  sender = $1;`
	...
	for rows.Next() {
		...
		// Pull data into message
		err := rows.Scan(&m.Sender, &m.Reciver)
		if err != nil {
			return nil, err
		}
